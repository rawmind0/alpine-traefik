...
TRAEFIK_CONSUL_CATALOG_ENABLE=${TRAEFIK_CONSUL_CATALOG_ENABLE:-"false"}
TRAEFIK_CONSUL_CATALOG_DOMAIN=${TRAEFIK_CONSUL_CATALOG_DOMAIN:-"consul.localhost"}
TRAEFIK_CONSUL_CATALOG_ENDPOINT=${TRAEFIK_CONSUL_CATALOG_ENDPOINT:-"127.0.0.1:8500"}
TRAEFIK_CONSUL_CATALOG_EXPOSED=${TRAEFIK_CONSUL_CATALOG_EXPOSED:-"false"}
TRAEFIK_CONSUL_CATALOG_CONSTRAINTS=${TRAEFIK_CONSUL_CATALOG_CONSTRAINTS:-""}
TRAEFIK_CONSUL_CATALOG_PREFIX=${TRAEFIK_CONSUL_CATALOG_PREFIX:-"traefik"}
TRAEFIK_CONSUL_CATALOG_OPTS=${TRAEFIK_CONSUL_CATALOG_OPTS:-""}
...
if [ "${TRAEFIK_CONSUL_CATALOG_ENABLE}" == "true" ]; then
    TRAEFIK_CONSUL_CATALOG_OPTS="\
[consulCatalog]
  domain = \"${TRAEFIK_CONSUL_CATALOG_DOMAIN}\"
  Watch = true
  prefix = \"${TRAEFIK_CONSUL_CATALOG_PREFIX}\"
  ExposedByDefault = ${TRAEFIK_CONSUL_CATALOG_EXPOSED}
  endpoint = \"${TRAEFIK_CONSUL_CATALOG_ENDPOINT}\"
"
    if [ "${TRAEFIK_CONSUL_CATALOG_CONSTRAINTS}" != "" ]; then
        TRAEFIK_CONSUL_CATALOG_OPTS=${TRAEFIK_CONSUL_CATALOG_OPTS}"\
  constraints = [ ${TRAEFIK_CONSUL_CATALOG_CONSTRAINTS} ]
"
    fi
fi
